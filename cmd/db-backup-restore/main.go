...
func main() {
	...
	if args.configPath == "" {
		utils.Fatalf("必须指定配置文件路径: -config <配置文件路径>")
	}

	cfg, err := config.LoadConfig(args.configPath)
	if err != nil {
		utils.Fatalf("加载配置文件失败: %v", err)
	}
	...
	dbCfg, err := cfg.GetDBConfig(args.dbType)
	if err != nil {
	...
	if err := executeAction(ctx, db, args, cfg); err != nil {
...
func executeAction(ctx context.Context, db backup.DatabaseBackup, args *Args, cfg *config.Config) error {
	backupTarget := cfg.BaseBackupDir + "\\" + args.dbType + "\\backup"
	archiveLogDest := cfg.BaseBackupDir + "\\" + args.dbType + "\\archivelog"
